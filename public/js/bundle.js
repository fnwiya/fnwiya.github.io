...
var _myHoneycombs = __webpack_require__(2);

var _myHoneycombs2 = _interopRequireDefault(_myHoneycombs);

var _mySlick = __webpack_require__(3);

var _mySlick2 = _interopRequireDefault(_mySlick);

var _myT_top = __webpack_require__(4);

var _myT_top2 = _interopRequireDefault(_myT_top);

var _smooth_scroll = __webpack_require__(6);
...
var honeycombs = function honeycombs() {
    (0, _jquery2.default)('.honeycombs').honeycombs({
...
var slick = function slick() {
    (0, _jquery2.default)('.works-panels').slick({
        infinite: true,
        dots: true,
        autoplay: true,
        autoplaySpeed: 4000,
...
var T_top = function T_top() {
  (0, _jquery2.default)('#map').t({
    speed: 10,
    speed_vary: true,
    delay: 0,
    mistype: 25,
  ...
  $.fn.t = function (content, opts) {
    ...
    return this.each(function () {
      ...
      var _o = opts,
          ...
          q2 = '12qwertzuiop\xFC+asdfghjkl\xF6\xE4#<yxcvbnm,.-!"\xA7$%&/(()=?1234567890\xDF*p+',
          t = function t(_1, _2) {
      ...
      if (c.data('is_typing')) {
        ...
        if (_c == 'pause') return this;c.data('is_typing', 1);
      }
      ...
      d = String(a || d).replace(/(.*?)[\u200b]+$/, '$1').replace(/<\!\-\-([\s\S]+?)\-\->/g, '$1').replace(/<ins><\/ins>/g, '').replace(/<(img|br|input|hr)(.*?)[\/]?>/g, '<#$1$2/>').replace(/<ins>\s*(\d+[.]?\d{0,})\s*<\/ins>/g, '<ins data-ins="$1"></ins>').replace(/<ins>([\s\S]+?)<\/ins>/g, function (a, b) {
        return '<#ins>' + b.replace(/<[^#]/g, '<#') + '</ins>';
      }).replace(/<(del.*?data-del=")(.*?)(".*?)><\/(del>)/g, '<#$1$2$3><#/$4$2<#/$4').replace(/<del(.*?)>([\s\S]*?)<\/del>/g, function (a, b, c, d) {
        i = c.match(/data-ins=\"(.*?)\"/);i = i && i[1] ? i[1] : 0.25;
        c = c.replace(/<[\/]?.*?>/g, '');return '<del' + b + ' data-del="' + c.replace(/\n/, '\\n') + '" data-ins="' + i + '">' + (c != '*' ? c : '') + '</del>';
      }).replace(/<(\w+)(.*?)>/g, '<$1$2></$1>').replace(/<#/g, "<").match(/<ins>[\s\S]+?<\/ins>|<[^<]+\/>|<\/\w+>|<[\s\S]+?><\/\w+>|&[#x]?[a-z0-9]+;|\r|\n|\t|\S|\s/ig);
      ...
      $$ = setInterval(function () {
          ...
          if (t(k, 'a')) {
            if (!k.pop()) {
              if (o == '*') {
                ...
                if (a) {
                  d = $.merge(['<' + _o.tag + ' class="typing-add"></' + _o.tag + '>'], d, ['</' + _o.tag + '>']);c = c.parent();
                }
              }c = c.parent();
              o == z && c.find(':last').remove();k = '';
            } else {
              c.text(k.join([]));tt = !tt;return;
            }
          }
        }

        while (k[1] == '/') {
          c = c.parent();k = d.shift() || '';
        }c.append(k);_o.typing($$$.parent(), f(), dl, k);!k.match(/<ins>/) && k.match(/></) && (c = c.find(':last'));

        if (!(k = d.shift())) {
          clearInterval($$);$$$.parent().removeData(['is_typing', 'pause']);
          t(_o.repeat) && --_o.repeat;if (_o.repeat > -1) {
            _o.init = $.noop;$$$.parent().t(t(_c, 'o') ? _o : _c, _o);return;
        ...
        }if (k && k[1] == '/' && c.data().ins) k = ~~(Number(c.data().ins) * 1e3 / _o.speed);tt = !tt;
      }, _o.speed);
    });
  };
})(jQuery);
